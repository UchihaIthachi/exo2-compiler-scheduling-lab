...
# --- AVX2 intrinsics (same as quiz1.py) --------------------------------------
...
def schedule(p: Procedure):
    """
    Quiz1 fix:
    replace_all(...) is memory-aware. The staged *_vec buffers are DRAM by default,
    so AVX2 instructions don't match unless we set them to AVX2 first.
    """
    ...
    # ✅ critical: set vector memories BEFORE replace_all
